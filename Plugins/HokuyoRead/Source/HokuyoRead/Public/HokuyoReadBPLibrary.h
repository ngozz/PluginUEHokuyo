...
class UHokuyoReadBPLibrary : public UBlueprintFunctionLibrary
{
	...
	//open the connection
	UFUNCTION(BlueprintCallable)
	static bool openConnection(UPARAM(ref) FUrgWrapper& UrgWrapper); //UPARAM(ref) is used to pass a reference to the struct. UE doesn't support references to structs by default using '&'.

	//get the distance
	UFUNCTION(BlueprintCallable)
	static bool getDistance(UPARAM(ref) FUrgWrapper& UrgWrapper);
